...
for FOCUS_DISTRICT in tqdm(UP_VILLAGES.dtname.unique()):
    ...
    for idx, row in tenders_df_FOCUSDISTRICT.iterrows():
        ...
        tender_village_id = ""
        tender_block = ""
        tender_block_location = ""      # ← initialize it here
        tender_gp = ""
        tender_subdistrict = ""
        tender_gp_name = ""
        ...
        for gp in FOCUSDIST_gp:
            ...
            pattern = r'\b{}\b'.format(re.escape(gp_search.strip()))
            if re.search(pattern, tender_slug, flags=re.IGNORECASE):
                tender_gp = gp
                break
...
MASTER_DF.to_csv(os.getcwd()+r'/Sources/TENDERS/data/floodtenders_subdistrictgeotagged.csv')
